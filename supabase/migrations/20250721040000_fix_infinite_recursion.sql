...
-- Temporarily disable RLS on profiles to fix the issue
ALTER TABLE public.profiles DISABLE ROW LEVEL SECURITY;

-- Also disable RLS on college_admin_requests
...
-- Create a simple function to insert college admin requests without RLS issues
CREATE OR REPLACE FUNCTION public.create_college_admin_request(
  p_user_id UUID,
  p_college_name TEXT,
  p_college_code TEXT,
...
-- Update the user registration trigger to be simpler
...
-- Create simple approval function
...
-- Create simple rejection function
CREATE OR REPLACE FUNCTION public.reject_college_admin_request(request_id UUID)
RETURNS BOOLEAN
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
